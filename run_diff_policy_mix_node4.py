...
def run_diff_policy(dat_dir, sol_dir, result_dir, set_path, fk_file_list, fk, exe="collect_multi_trjs_new", timelimit=3600):
    signal.signal(signal.SIGINT, signal_handler)
    print("数据集：", dat_dir)
    print(sol_dir)
    print("日志路径：", result_dir)
    ...
    # for j, ins_file in enumerate(file_list):
        ...
        os.system(f"bin/scip6_dl_pair_hxl_set_node4 -s ./sets/fullstrong_bfs.set -f %s > %s" % (f_path, r_path))
            # os.system(f"bin/scip6_ins_set_6_node11 -s ./sets/fullstrong_bfs.set -f %s > %s" % (f_path, r_path))
            # os.system(f"bin/scip6_ins_fac_acc_pol3_{fk} -s ./sets/fullstrong_bfs.set -f %s > %s" % (f_path, r_path))
        # except KeyboardInterrupt:
        #     print("numpolicy %d %d %s stopped" % (0, j, ins_file))
        #     continue

def run_scip(dat_dir, sol_dir, result_dir, set_path, exe="collect_multi_trjs_new", timelimit=3600, first_k=20, dagger="scip"):
    signal.signal(signal.SIGINT, signal_handler)
...
if __name__ == "__main__":
    ...
    # training_files_base="/home/xlm/temp_xlm_11_all/daggerSpace/training_files/scip-dagger"
    training_files_base="/home/xuliming/xuliming/xuliming/daggerSpace/training_files/scip-dagger"
    ...
    fk_file_list = file_list[:fk]
        # p_dir = os.path.join(training_files_base,"trained_models", dat_type, dat_name, experiment,"/2022-03-15-15-29_nodelist/insL200_trjL5e4")
    if args.dagger == 'policy':
        ...
        run_diff_policy(dat_dir=d_path, sol_dir=s_dir, result_dir=r_dir, set_path=set_name, fk_file_list=fk_file_list, fk=fk)
    else:
        run_scip(dat_dir=d_path, sol_dir=s_dir, result_dir=r_dir, set_path=set_name, first_k=fk, dagger=args.dagger)
